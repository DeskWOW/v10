...
// =====================================================
  if (currentPage == 'page_index') {
    if (deskEV('enable_gs') == 'true') {
      ...
      setTimeout(function(){
       $("#gs_questions div.topic h5 a").appendTo("#gs_questions div.topic").addClass("button round");
       $("#gs_ideas div.topic h5 a").appendTo("#gs_ideas div.topic").addClass("button round");
       $("#gs_problems div.topic h5 a").appendTo("#gs_problems div.topic").addClass("button round");
       $("#gs_praises div.topic h5 a").appendTo("#gs_praises div.topic").addClass("button round");
      }, 3500);
    }
  };
// =====================================================
            ...
            function apiSuccess(data, brandID, brandName) {
                ...
                if (as_count > 0) {
                  ...
                  if (as_count > 0) {
                    ...
                    if (as_count > 9) {
                      $('.autosuggest.multi-brand div#brand-' + brandID + ' ul div').append('<a class="button button-submit" target="_blank" href="//' + document.domain.toString() + '/customer/' + systemLanguageDesk + '/portal/articles/search?b_id=' + brandID + '&q=' + search_query + '&displayMode=BrandOnly">View All</a>');
                    }
                    $('.autosuggest.multi-brand div#brand-' + brandID + ' ul div').append('</div>');
                    if(as_count > 3) {
                      $('.autosuggest.multi-brand div#brand-' + brandID + ' ul').append('<button class="button button-submit coltrig">More</button>');
                    }
                  } // IF SUGGESTIONS
                  as_count = 0;
                }
...
// =====================================================
  if (currentPage == 'login') {
    ...
    $(".alternatelogins a[href*='facebook']").prepend('<i class="fa fa-facebook fa-lg"></i> ').addClass('button').after('<br />');
    $(".alternatelogins a[href*='twitter']").prepend('<i class="fa fa-twitter fa-lg"></i> ').addClass('button');
...
// =====================================================
  if (currentPage == 'myaccount') {
    ...
    jQuery(document).ready(function() {
        ...
        $(".myportal #add_facebook input").addClass('button');
        $(".myportal #add_twitter input").addClass('button');
...
// =====================================================
  if (currentPage == 'customer_feedback') {
    ...
    $('.button.value-' + VoteValue ).addClass('active');
    //FEEDBACK BUTTONS
    $(function () {
      $('.button-radio').click(function(e) {
        $('.button-radio').not(this).removeClass('active')
          ...
          //ADDING TAB ELEMENTS FOR BRANDS AND ALL RESULTS
            ...
            MultiSearch = function(data) {
              ...
              $.each(data, function() {
                ...
                auto_suggest_articles += '<article class="row nomarg result article"><div class="columns medium-12"><h3><a href="' + this.id + '&t=' + themeID + '">' + article_title + '</a></h3><p>' + article_body + '</p><a class="button round" href="' + this.id + '">' + readArticle + '</a></div></article>';
                resultsCount++;
              });
              ...
              if (resultsCount >= 10) {
                $('#siteResults div.tab-content div#' + brandID + 'Results .footer').append('<button class="next-page button button-primary" data-page="2" data-brand="'+brandID+'">Load More Results<i class="fa fa-spinner fa-spin hidden"></i></button>');
              }
            };
          //NO RESULTS
                  ...
                  if (as_count > 9) {
                    $('#SearchAutoSuggest').append(' ' +
                       '<div class="columns medium-12" id="brand-' + brandID + '">' +
                        ...
                        '<a class="button button-submit" target="_blank" href="//' + document.domain.toString() + '/customer/' + systemLanguageDesk + '/portal/articles/search?b_id=' + brandID + '&q=' + search_query + '">View All</a></div>' +
